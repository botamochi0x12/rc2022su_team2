...
module Directors
	...
	class Result_wAttacker < Base
		...
		def initialize(renderer:, aspect:)
			...
			@Win_Attacker = TextBoard.new(texture_path: "textures/Win_Attacker.png",value: "Win_Attacker",x:0,y:0,z:7,w:10,h:5)#
			#@Win_Attacker.mesh.position.x
			#@Win_Attacker.mesh.scale_y = 8
			self.scene.add(@Win_Attacker.mesh)
