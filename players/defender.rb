...
module Players
    ...
    class Defender < Base
        ...
        def play(key_statuses, _selected_mode)
            ...
            mesh.position.x -= 0.1 if key_statuses[control_keys[0]]
            mesh.position.x += 0.1 if key_statuses[control_keys[1]]
            mesh.position.z -= 0.1 if key_statuses[control_keys[2]]
            mesh.position.z += 0.1 if key_statuses[control_keys[3]]

            if mesh.position.y > Directors::Game::DEFENDER_LEVEL
                @speed -= 0.1
            elsif key_statuses[control_keys[4]]
                @speed = 1.0
            end
            mesh.position.y += @speed
            if mesh.position.y <= Directors::Game::DEFENDER_LEVEL # 地面に埋まっていたら地面より上にあげる
        ...
        def intercept_bombs(bombs = [])
            ...
            @raycaster.set(mesh.position, @norm_vector)
